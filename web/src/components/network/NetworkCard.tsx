...
import { formatDisplayAddress } from "@/lib/network/derive-location-name";
...
export function NetworkCard({ repairer }: { repairer: Repairer }) {
  ...
  const displayAddress = formatDisplayAddress(repairer.address, repairer.name);

  return (
    <article className="flex min-h-[130px] flex-col rounded-[10px] border !border-[#e8eef5] bg-white px-5 py-6 shadow-[0_8px_24px_rgba(7,29,51,0.06)] transition hover:!border-[var(--color-button-blue)] hover:ring-1 hover:!ring-[rgba(30,115,190,0.15)]">
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 flex-1 gap-4">
          ...
          <div className="min-w-0">
            ...
            {repairer.email?.trim() ? (
              <a
                href={`mailto:${repairer.email.trim()}`}
                className="mt-2 inline-flex w-fit items-center gap-1.5 text-[13px] font-semibold leading-5 !text-[#247dca] transition hover:!text-[#1b6eb5]"
              >
            ...
            <a
              ...
              className="mt-3 inline-flex w-fit items-center gap-1 text-[11px] font-bold leading-none !text-[#247dca] transition hover:gap-2 hover:!text-[#1b6eb5]"
            >
