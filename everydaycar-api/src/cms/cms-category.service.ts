...
export class CmsCategoryService {
   ...
   */
  async findAll(queryDto: FindAllCmsCategoryQueryDto): Promise<{
    results: CmsCategory[];
    total: number;
    page: number;
    limit: number;
  ...
  async findList(filters?: {
    ...
    const activeOnly =
      filters?.status === true ||
      (filters?.status === undefined && Boolean(filters?.placement));

    if (filters?.status !== undefined) {
      query.status = filters.status;
    } else if (filters?.placement) {
      query.status = true;
    }
    ...
    let categories = await this.cmsCategoryModel
      .find(query)
    ...
    if (activeOnly) {
      ...
      const categoriesWithActivePages = await this.pageModel.distinct(
        "category",
        {
          category: { $in: slugs },
          status: true,
      ...
      const activeSlugSet = new Set(categoriesWithActivePages);

      categories = categories.filter(
        (category) => category.slug && activeSlugSet.has(category.slug),
      );
    }
