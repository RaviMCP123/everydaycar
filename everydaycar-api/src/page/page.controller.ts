...
import { preparePageHtmlForSave } from "../helpers/html-content.helper";
import {
  replaceBase64ImagesInHtml,
  saveUploadedImageFile,
} from "../helpers/upload-path.helper";
...
export class PageController {
  ...
  /**
   * Uploads an image for CKEditor / rich text fields.
   * Files are stored under `public/uploads` and served at `/uploads/{filename}`.
   */
  @Post("upload-image")
  ...
  async uploadEditorImage(
    ...
    const url = saveUploadedImageFile(file);
    return res.status(HttpStatus.OK).json({
      statusCode: HttpStatus.OK,
      message: i18n.t("messages.DATA_SAVED") || "Image uploaded",
      data: { url },
    });
  }
  ...
  async addPage(
    ...
    const isBlogPage =
      pageData.templateKey === "BLOG_TEMPLATE_V1" ||
      pageData.category === "blog";
    ...
    if (isBlogPage && (!titleString || titleString.trim() === "")) {
      if (
        ...
        Array.isArray(pageData.blogSections) &&
        pageData.blogSections.length > 0
      ) {
      ...
      if (
        !getStringValue(pageData.title) ||
        getStringValue(pageData.title).trim() === ""
      ) {
    ...
    if (isBlogPage && (!descriptionString || descriptionString.trim() === "")) {
      ...
      if (
        !getStringValue(pageData.description) ||
        getStringValue(pageData.description).trim() === ""
      ) {
    ...
    if (pageData.bannerDescription) {
      if (typeof pageData.bannerDescription === "string") {
        try {
          (pageData as any).bannerDescription = JSON.parse(
            pageData.bannerDescription,
          );
        } catch {
          // If not JSON, convert to object format
          (pageData as any).bannerDescription = {
            en: pageData.bannerDescription,
          };
        }
      }
    }
    ...
    if (pageData.metaDescription) {
      if (typeof pageData.metaDescription === "string") {
        try {
          (pageData as any).metaDescription = JSON.parse(
    ...
    if (
      ...
      try {
        ...
        try {
          ...
          const scriptMatch = pageData.schemaMarkup.match(
            /<script[^>]*>([\s\S]*?)<\/script>/i,
          );
          if (scriptMatch && scriptMatch[1]) {
            ...
            console.warn(
              "Invalid JSON in schemaMarkup, but keeping original value",
            );
          }
        } catch {
          // If both fail, log warning but keep the original value (don't set to empty)
          console.warn(
            "Could not validate JSON in schemaMarkup, but keeping original value",
          );
        }
      }
    }
    ...
    if (pageData.robotsIndex !== undefined) {
      (pageData as any).robotsIndex =
        pageData.robotsIndex === true ||
        String(pageData.robotsIndex) === "true";
    }
    if (pageData.robotsFollow !== undefined) {
      (pageData as any).robotsFollow =
        pageData.robotsFollow === true ||
        String(pageData.robotsFollow) === "true";
    }

    pageData = await this.applyPageUploads(pageData, files);
  ...
  async updatePageById(
    ...
    const updatedPage = await this.pageService.updatePage(pageData, pageId);
    await this.auditLogService.createAudit({
  ...
  async updateStatus(
    ...
    return res.status(HttpStatus.OK).json({
      statusCode: HttpStatus.OK,
      message:
        i18n.t("messages.DATA_UPDATED") || "Page status updated successfully",
      data: updatedPage,
    });
  }
   ...
   */
  private async applyPageUploads(
    pageData: any,
    files?: Express.Multer.File[],
  ): Promise<any> {
    if (typeof pageData.description === "string") {
      pageData.description = replaceBase64ImagesInHtml(pageData.description);
      const prepared = preparePageHtmlForSave(
        pageData.description,
        typeof pageData.customCss === "string" ? pageData.customCss : "",
      );
      pageData.description = prepared.description;
      pageData.customCss = prepared.customCss;
    } else if (
      typeof pageData.customCss === "string" &&
      pageData.customCss.trim()
    ) {
      pageData.customCss = pageData.customCss.trim();
    }
  ...
  private async processBannerImage(
    ...
    for (const file of files) {
      if (file.fieldname === "bannerImage") {
        ...
        pageData.bannerImage = saveUploadedImageFile(file);
        break;
      }
    }
  ...
  private async processFeaturedImage(
    ...
    for (const file of files) {
      if (file.fieldname === "featuredImage") {
        ...
        pageData.featuredImage = saveUploadedImageFile(file);
        break;
      }
    }
      ...
      if (file.fieldname && file.fieldname.startsWith("pageSection-")) {
        ...
        if (match) {
          ...
          if (
            ...
            while (
              processedSections[sectionIndex].images.length <= imageIndex
            ) {
              processedSections[sectionIndex].images.push("");
            }

            processedSections[sectionIndex].images[imageIndex] =
              saveUploadedImageFile(file);
          }
        }
      }
    }
        ...
        if (key === "_id" && val && typeof val === "object") {
          ...
          if (val.buffer && val.buffer.type === "Buffer") {
            try {
              ...
              if (
                val.buffer.data &&
                Array.isArray(val.buffer.data) &&
                val.buffer.data.length > 0
              ) {
                converted[key] = new Types.ObjectId(
                  Buffer.from(val.buffer.data),
                ).toString();
  ...
  async getPageDetail(
    ...
    const obj =
      result && typeof result === "object" && "toObject" in result
        ? result.toObject()
        : result;
  ...
  async deletePage(
    ...
    try {
      ...
      return res.status(HttpStatus.OK).json({
        statusCode: HttpStatus.OK,
        message:
          i18n.t("messages.DELETE_SUCCESS") || "Page deleted successfully",
        data: {},
      });
    } catch (error) {
      if (
