...
export class PageService {
  ...
  async createPage(request: PageDto): Promise<Page> {
    ...
    if (request.title && typeof request.title === "object") {
      ...
      titleString =
        (request.title as any).en ||
        Object.values(request.title as any)[0] ||
        "";
    }
    ...
    if (request.description && typeof request.description === "object") {
      // Extract 'en' value or first available value
      descriptionString =
        (request.description as any).en ||
        Object.values(request.description as any)[0] ||
        "";
    }
    ...
    // Only activate category when the first page is created as active.
    // Category status is otherwise controlled from the CMS category admin.
    if (request.category && savedPage.status !== false) {
  ...
  private async updateCategoryStatus(
    ...
    try {
      ...
      if (category && category.status !== status) {
        await this.cmsCategoryModel.findByIdAndUpdate(category._id, {
          $set: { status: status },
        });
      }
    } catch (error) {
      ...
      if (result && result._id) {
        ...
        if (idValue && typeof idValue === "object") {
          if (idValue.toString && typeof idValue.toString === "function") {
            ...
            idValue.buffer &&
            idValue.buffer.data &&
            Array.isArray(idValue.buffer.data) &&
            idValue.buffer.data.length > 0
          ) {
            result._id = new Types.ObjectId(
              Buffer.from(idValue.buffer.data),
            ).toString();
          } else if (idValue.hex) {
  ...
  async updatePageStatus(
    ...
    const page = await this.pageModel.findById(id).select("category").lean();
    await this.pageModel.updateOne({ _id: id }, request);

    if (page?.category) {
      await this.syncCategoryStatusFromPages(page.category);
    }
  ...
  /**
   * Sets category inactive when no published pages remain; never forces active.
   */
  private async syncCategoryStatusFromPages(
    categorySlug: string,
  ): Promise<void> {
    try {
      const activePageCount = await this.pageModel.countDocuments({
      ...
      console.error("Error syncing category status from pages:", error);
    }
  }
