...
import { useGetNetworkAddressListQuery } from "@services/networkAddressApi";
...
import {
  ...
  getDefaultTemplateKeyForCategory,
...
type NetworkRegionItem = {
  regionTitle: Record<string, string>;
  addresses: NetworkRegionAddressItem[];
};

type NetworkAddressListItem = {
  _id?: string;
  id?: string;
  regionId?:
    | string
...
const Index: React.FC<FormProps> = ({ isOpen, closeModal, item, existingPages = [] }) => {
  ...
  const inferredTemplate = getDefaultTemplateKeyForCategory(watchedCategory);
  const currentTemplateKey =
    watchedCategory === "home"
      ? "home_template"
      : ((watchedTemplateKey as string) || item?.templateKey || inferredTemplate);
  const selectedTemplate = currentTemplateKey;
  const templateConfig = getTemplateByKey(currentTemplateKey);
  const isTemplateMode = !!templateConfig;
  const isHomePage = currentTemplateKey === "home_template";
  const isPageTemplate =
    (currentTemplateKey === "page_template" ||
      currentTemplateKey === "portfolio_template" ||
      currentTemplateKey === "PAGE_TEMPLATE_V1") &&
    !isHomePage;
  ...
  const isContactUs = isContactContentTemplate(currentTemplateKey) || watchedCategory === "contact-us";
  const isRegisterSchool = watchedCategory === "register-school";
  const isFaq = watchedCategory === "faq";
  const isTermsCondition = watchedCategory === "terms-condition";
  const isFooterTemplate = isFooterContentTemplate(currentTemplateKey) || watchedCategory === "footer";
  ...
  const managerDrivenNetworkRegions = React.useMemo<NetworkRegionItem[]>(() => {
    const rows = ((networkAddressListData as any)?.data || []) as NetworkAddressListItem[];
    const grouped = new Map<string, NetworkRegionItem>();
    rows.forEach((row) => {
      const regionName =
      ...
      if (!grouped.has(normalizedRegion)) {
        grouped.set(normalizedRegion, {
          regionTitle: { en: normalizedRegion },
          addresses: [],
        });
      }
      const item = grouped.get(normalizedRegion)!;
      item.addresses.push({
  ...
  useEffect(() => {
    ...
    if (isEditing) {
          ...
          Object.entries(item.content).forEach(([key, value]) => {
            if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
              ...
              if (value.length === 0) {
                ...
                // Preserve structured arrays (e.g. network regions with addresses) as-is.
                normalizedContent[key] = value;
              }
            }
          });
        ...
        const itemTemplateKey =
          item.templateKey || getDefaultTemplateKeyForCategory(item.category);
        const itemIsNetworkTemplate =
          itemTemplateKey === "network_template" ||
          itemTemplateKey === "our_network_template";

        if (itemIsNetworkTemplate) {
          const loadedRegionsRaw = Array.isArray(normalizedContent.networkRegions)
            ? (normalizedContent.networkRegions as NetworkRegionItem[])
            : [];
          const loadedRegions = loadedRegionsRaw.map((region) => ({
            ...region,
            addresses: (region.addresses || []).map((addr) => ({
              ...addr,
              status: typeof addr.status === "string" && addr.status.trim() ? addr.status : "Approved",
            })),
          }));
          setNetworkRegions(loadedRegions);
        } else {
          setNetworkRegions([]);
        }
        ...
        const itemCustomCss = (item as any)?.customCss;
        setCustomCss(
          typeof itemCustomCss === "string" ? itemCustomCss : "",
        );
  ...
  const categoryOptions = React.useMemo(() => {
    // For create mode, if filtered list becomes empty, allow selecting from all categories.
    // This prevents the form from being blocked when categories are already used.
  ...
  useEffect(() => {
    ...
    const currentTemplateKey = watch("templateKey");
    if (!currentTemplateKey && STATIC_PAGE_TEMPLATES[0]) {
      setValue("templateKey", STATIC_PAGE_TEMPLATES[0].value, {
        shouldDirty: false,
        shouldTouch: false,
      });
    }
  }, [isOpen, isEditing, setValue, watch]);
  ...
  const onSubmit: SubmitHandler<FormValuesInterface> = async (
    ...
    if (isNetworkTemplate) {
      const effectiveNetworkRegions =
        managerDrivenNetworkRegions.length > 0
          ...
          : networkRegions;
      const hasRegion = effectiveNetworkRegions.length > 0;
      if (!hasRegion) {
        showToast("Please add regions/addresses in Network managers first.", "error");
        return;
      }
      const hasInvalidRegion = effectiveNetworkRegions.some((region) => {
        const regionTitleFilled = Object.values(region.regionTitle || {}).some(
          (v) => String(v || "").trim() !== "",
        );
        if (!regionTitleFilled) return true;
        if (!Array.isArray(region.addresses) || region.addresses.length === 0) return true;
        return region.addresses.some(
          (row) => !Object.values(row.address || {}).some((v) => String(v || "").trim() !== ""),
        );
      });
      if (hasInvalidRegion) {
        showToast(
          "Each region needs a title and at least one address.",
          "error",
        );
        return;
      }
    ...
    if (isNetworkTemplate) {
      // Auto-fill from Network Address Manager (grouped by selected Region)
      // Fallback to locally edited values only if manager has no data.
      templateContent.networkRegions =
        managerDrivenNetworkRegions.length > 0
          ? managerDrivenNetworkRegions
          : networkRegions;
    }
    ...
    const selectedTemplateKey = isHomePage ? "home_template" : currentTemplateKey;

    // Home pages use template `content` as source of truth (no CKEditor description editing).
    let descriptionValue: string | Record<string, string> =
      isHomePage && typeof item?.description === "string" ? item.description : " ";
    ...
    let preparedCustomCss = customCss.trim();
    if (!isHomePage && typeof descriptionValue === "string") {
      const prepared = preparePageHtmlForSave(descriptionValue, customCss);
      descriptionValue = prepared.description;
      preparedCustomCss = prepared.customCss;
    }

    // Handle banner image (page template) and home hero image (home template)
    ...
    if (isHomePage && templateImageFiles.heroImage?.[0]?.originFileObj) {
      formDataFiles.heroImage = templateImageFiles.heroImage[0].originFileObj;
    }
    if (isHomePage && templateImageFiles.joinIcon?.[0]?.originFileObj) {
      formDataFiles.joinIcon = templateImageFiles.joinIcon[0].originFileObj;
    }
    for (const i of [1, 2, 3, 4]) {
      const key = `trustItem${i}Icon`;
      if (templateImageFiles[key]?.[0]?.originFileObj) {
        formDataFiles[key] = templateImageFiles[key][0].originFileObj!;
      }
    }
    ...
    const seoFields = {
      ...
      customCss: preparedCustomCss || undefined,
  ...
  const isSaveDisabled = React.useMemo(() => {
    ...
    return categoryEmpty || isTitleEmpty;
  }, [watchedCategory, isTitleEmpty, isEditing, item?.category]);

  return (
    <Modal isOpen={isOpen} onClose={closeModal} className={`m-4 ${isTemplateMode ? 'max-w-[95vw]' : isFaq ? 'max-w-[900px]' : 'max-w-[1100px]'}`} preventClose={isLoading}>
      <div className={`no-scrollbar relative w-full overflow-hidden rounded-3xl bg-white dark:bg-gray-900 ${isTemplateMode ? 'max-w-[95vw] h-[95vh]' : isFaq ? 'max-w-[900px] h-[95vh]' : 'max-w-[1100px] h-[90vh]'} flex flex-col`}>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            ...
            <div className="px-6 pt-4 pb-3 flex-shrink-0 border-b border-gray-200 dark:border-gray-700">
                  ...
                  {...register("category", {
                    ...
                    onChange: (e) => {
                      ...
                      if (selectedCategoryValue) {
                        ...
                        // Keep template content empty for new pages; admin enters values manually.
                        if (!isEditing) {
                          const nextTemplateKey =
                            getDefaultTemplateKeyForCategory(selectedCategoryValue);
              ...
              <Form.Group className="mt-4">
                <Label>
                  Template <span className="text-error-500">*</span>
                </Label>
                <Form.Control
                  as="select"
                  className="h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs focus:outline-hidden focus:ring-3 dark:bg-gray-900 dark:text-white/90 bg-transparent text-gray-800 border-gray-300 focus:border-brand-300 focus:ring-brand-500/20 dark:border-gray-700"
                  {...register("templateKey", {
                    ...
                    onChange: (e) => {
                      const nextTemplateKey = e.target.value as string;
                  ...
                  value={watch("templateKey") || currentTemplateKey || inferredTemplate}
                  isInvalid={!!errors?.templateKey}
                  disabled={isEditing}
                >
                  <option value="">Select Template</option>
                  {STATIC_PAGE_TEMPLATES.map((tpl) => (
                    <option key={tpl.value} value={tpl.value}>
                      {tpl.label}
                    </option>
                  ))}
                </Form.Control>
                <Message message={(errors as any)?.templateKey?.message ?? ""} />
              </Form.Group>
            </div>
            ...
            {isTemplateMode && templateConfig ? (
              <div className="pt-4 flex-1 flex flex-col min-h-0 overflow-y-auto px-6">
                ...
                {isNetworkTemplate ? (
                  <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-xs text-gray-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                    Network addresses are managed from <strong>Network Regions</strong> and{" "}
                    <strong>Network Addresses</strong> managers.
                  </div>
                ) : null}
              </div>
            ) : null}

            {/* Template-based pages are fully structured; hide freeform description/custom CSS block. */}
            {!isTemplateMode ? (
            <>
                  ...
                  <Label>Description</Label>
                    ...
                    <Form.Group className="mb-4">
                      ...
                      <Form.Control
                        ...
                        placeholder=".cms-html-content h2 { color: #007bff; }"
                      />
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Optional page styles. Any &lt;style&gt; tags pasted in the description are extracted and saved here automatically.
                      </p>
                    </Form.Group>
            </div>
            </>
